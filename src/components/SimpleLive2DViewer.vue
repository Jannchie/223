...
const canvasScale = useLocalStorage('live2d-canvas-scale', 1.0);
const minScale = 0.1;
const maxScale = 3.0;
...
const getModelURL = () => {
  ...
  if ((window as any).electronAPI) {
    return (window as any).electronAPI.getModelPath(model_path)
  }
  // 开发模式下的回退路径
  return `${window.location.origin}/models/${model_path}`
}
...
// 组件卸载时清理事件监听器
onUnmounted(() => {
  if ((window as any).electronAPI && (window as any).electronAPI.removeMousePositionListener) {
    (window as any).electronAPI.removeMousePositionListener();
  }
});
</script>
